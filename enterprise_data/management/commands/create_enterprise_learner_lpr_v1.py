...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            info = f"Error trying to create EnterpriseLearner with uuid {enterprise_customer_uuid}: {exc}"
            raise CommandError(info) from exc

        info = f"\n\nCreated EnterpriseLearner with id {ent_user.enterprise_user_id} for enterprise with uuid {ent_user.enterprise_customer_uuid}\n"
        ...
        info = (
            ...
            f"command:\n\n    ./manage.py create_enterprise_learner_enrollment_lpr_v1 {ent_user.enterprise_customer_uuid} {ent_user.enterprise_user_id}\n\n"
        )
        sys.stdout.write(info)
