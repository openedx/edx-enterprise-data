...
from enterprise_data.admin_analytics.constants import (
    CALCULATION_MOVING_AVERAGE_3_PERIOD,
    CALCULATION_MOVING_AVERAGE_7_PERIOD,
    CALCULATION_RUNNING_TOTAL,
    GRANULARITY_MONTHLY,
...
def granularity_aggregation(level, group, date, data_frame, aggregation_type="count"):
    ...
    period_mapping = {
        GRANULARITY_WEEKLY: "W",
        GRANULARITY_MONTHLY: "M",
        GRANULARITY_QUARTERLY: "Q"
    }
...
def calculation_aggregation(calc, data_frame, aggregation_type="count"):
    ...
    window_mapping = {
        CALCULATION_MOVING_AVERAGE_3_PERIOD: 3,
        CALCULATION_MOVING_AVERAGE_7_PERIOD: 7,
    }

    aggregation_column = "count" if aggregation_type == "count" else "sum"

    if calc == CALCULATION_RUNNING_TOTAL:
        df[aggregation_column] = df.groupby("enroll_type")[aggregation_column].cumsum()
    elif calc in [CALCULATION_MOVING_AVERAGE_3_PERIOD, CALCULATION_MOVING_AVERAGE_7_PERIOD]:
