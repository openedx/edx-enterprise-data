    ...
    def get_queryset(self):
        ...
        if active_courses == "true":
            queryset = queryset.filter(
                Q(enrollments__consent_granted=True), enrollments__course_end__gte=timezone.now()
            )
        elif active_courses == "false":
            queryset = queryset.filter(
                Q(enrollments__consent_granted=True), enrollments__course_end__lte=timezone.now()
            )
...
class EnterpriseLearnerCompletedCoursesViewSet(EnterpriseViewSetMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        enrollments = (
            ...
            .values("user_email")
            .annotate(completed_courses=Count("course_id"))
            .order_by("user_email")
        )
        return enrollments
