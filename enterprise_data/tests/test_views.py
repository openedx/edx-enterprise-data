...
class TestEnterpriseEnrollmentsViewSet(JWTTestMixin, APITransactionTestCase):
    ...
    def test_get_queryset_returns_enrollments(self):
        ...
        self.mocked_get_enterprise_customer.return_value = {"uuid": enterprise_id}
            ...
            "results": [
                {
                    ...
                    "course_api_url": (
                        "/enterprise/v1/enterprise-catalogs/ee5e6b3a-069a-4947-bb8d-d2dbc323396c"
    ...
    def test_get_queryset_returns_enrollments_with_passed_date_filter(self):
        ...
        for enrollment, passed_date in zip(result["results"], in_past_week_passed_dates, strict=False):
    ...
    @ddt.data(
        ("active_past_week", [date.today(), date.today() - timedelta(days=2)]),
        ("inactive_past_week", [date.today() - timedelta(weeks=2), subtract_one_month(date.today())]),
        ("inactive_past_month", [subtract_one_month(date.today())]),
    )
    @ddt.unpack
    ...
    @ddt.data(("active_past_week",), ("inactive_past_week",), ("inactive_past_month",))
    @ddt.unpack
    ...
    def test_get_overview_returns_overview(self):
        enterprise_id = self.enterprise_id
        self.mocked_get_enterprise_customer.return_value = {"uuid": enterprise_id}
        url = reverse("v0:enterprise-enrollments-overview", kwargs={"enterprise_id": enterprise_id})
        expected_result = {
            "enrolled_learners": 2,
            "active_learners": {
                "past_week": 0,
...
class TestEnterpriseUsersViewSet(JWTTestMixin, APITransactionTestCase):
    ...
    def test_viewset_filter_has_enrollments_garbled(self):
        ...
        params = {
            "has_enrollments": "asdiqwjodijacvasd",
        }
        url = reverse(
            "v0:enterprise-users-list",
            kwargs=kwargs,
    ...
    def test_users_are_sortable_by_enrollment_count(self):
        ...
        params = {"extra_fields": "enrollment_count", "ordering": "enrollment_count"}
        response = self.client.get(url, params)
    ...
    def test_users_are_sortable_by_course_completion_count(self):
        ...
        params = {"extra_fields": "course_completion_count", "ordering": "course_completion_count"}
        response = self.client.get(url, params)
...
class TestEnterpriseLearnerCompletedCourses(JWTTestMixin, APITransactionTestCase):
    ...
    def setUp(self):
        ...
        enterprise_api_client = mock.patch(
            ...
            mock.Mock(
                return_value=mock.Mock(get_enterprise_customer=mock.Mock(return_value=get_dummy_enterprise_api_data()))
            ),
        )
        ...
        self.enterprise_id = "ee5e6b3a-069a-4947-bb8d-d2dbc323396c"
        self.enterprise_api_client.return_value.get_enterprise_customer.return_value = {"uuid": self.enterprise_id}
    ...
    def test_get_learner_completed_courses(self):
        ...
        expected_result = {
            ...
            "num_pages": 1,
            "current_page": 1,
            "results": [{"completed_courses": 1, "user_email": "test@example.com"}],
            "next": None,
            "start": 0,
            "previous": None,
        }
...
class TestEnterpriseEnrollmentsViewSetFilters(JWTTestMixin, APITransactionTestCase):
    ...
    def setUp(self):
        ...
        for course_date, title in zip(self.all_course_dates, self.all_course_titles, strict=False):
            EnterpriseEnrollmentFactory(
        ...
        for course_date, title in zip(
            [self.course_date_0, self.course_date_1], [self.course_title_0, self.course_title_1], strict=False
        ):
            EnterpriseEnrollmentFactory(
    ...
    @ddt.data(("2020-09-30T00:00:00Z", 2), ("2020-09-03T00:00:00Z", 3), ("2020-08-30T00:00:00Z", 1))
    @ddt.unpack
    ...
    def test_get_enrollments_multiple_filters(self, search_email, search_course, search_date, expected_count):
        ...
        response = self.client.get(
            reverse("v0:enterprise-enrollments-list", kwargs={"enterprise_id": self.enterprise_id}), data=data_dict
        ).json()
