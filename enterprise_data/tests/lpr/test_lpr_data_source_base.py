...
class TestLPRDataSourceInterface:
    ...
    def test_get_enrollments_raises_not_implemented_with_query_params(self):
        with pytest.raises(NotImplementedError):
            LPRDataSource().get_enrollments("test-uuid", limit=10, offset=0, query_params={"search": "alice"})
...
class TestNormalizeUuid:
    ...
    def test_standard_uuid_format(self):
        result = LPRSerializerShapeMixin.normalize_uuid("A1B2C3D4-E5F6-7890-ABCD-EF1234567890")
...
class TestEnrich:
    ...
    def test_only_serializer_fields_are_returned(self):
        ...
        for key in result:
            assert key in LPRSerializerShapeMixin.SERIALIZER_FIELDS, f"Unexpected field in enrich output: {key}"
