...
class EnterpriseEnrollmentFactory(factory.django.DjangoModelFactory):
    ...
    course_start = factory.lazy_attribute(
        lambda x: FAKER.date_time_between(  # pylint: disable=no-member
            start_date="-2M", end_date="+2M", tzinfo=pytz.utc
        )
    )
    user_email = factory.lazy_attribute(lambda x: FAKER.email())  # pylint: disable=no-member
    current_grade = factory.lazy_attribute(lambda x: FAKER.pyfloat(right_digits=2, min_value=0, max_value=1))  # pylint: disable=no-member

    @factory.lazy_attribute
    def course_end(self):
        return FAKER.date_time_between(  # pylint: disable=no-member
            start_date=self.course_start, end_date="+8M", tzinfo=pytz.utc
        )
    ...
    def passed_timestamp(self):
        ...
        if self.has_passed:
            return FAKER.date_time_between(  # pylint: disable=no-member
                start_date=self.course_start, end_date=self.course_end, tzinfo=pytz.utc
            )
        return None
...
class EnterpriseLearnerEnrollmentFactory(factory.django.DjangoModelFactory):
    ...
    enterprise_user = factory.SubFactory(EnterpriseLearnerFactory, enterprise_user_id=factory.Sequence(lambda n: n + 1))
    ...
    has_passed = factory.lazy_attribute(lambda x: FAKER.boolean())  # pylint: disable=no-member
    course_title = factory.lazy_attribute(lambda x: " ".join(FAKER.words(nb=2)).title())  # pylint: disable=no-member
    course_start_date = factory.lazy_attribute(
        lambda x: FAKER.date_between(  # pylint: disable=no-member
            start_date="-2M",
            end_date="+2M",
        )
    )
    ...
    def course_end_date(self):
        return FAKER.date_between(  # pylint: disable=no-member
            start_date=self.course_start_date, end_date="+8M"
        )
    ...
    def set_fields_according_to_consent(obj, create, extracted, **kwargs):  # pylint: disable=unused-argument, missing-function-docstring
...
class EnterpriseSubsidyBudgetFactory(factory.django.DjangoModelFactory):
    ...
    is_active = True
    is_test = False
    ocm_usage = factory.lazy_attribute(lambda x: round(x.amount_of_policy_spent / 2, 2))
    exec_ed_usage = factory.lazy_attribute(lambda x: round(x.amount_of_policy_spent / 2, 2))
    usage_total = factory.lazy_attribute(lambda x: x.amount_of_policy_spent)
    ...
    starting_balance = factory.LazyAttribute(lambda x: FAKER.pyfloat(right_digits=2, min_value=10000, max_value=100000))
    ...
    remaining_balance = factory.lazy_attribute(lambda x: x.starting_balance - x.amount_of_policy_spent)
    percent_of_policy_spent = factory.lazy_attribute(lambda x: round(x.amount_of_policy_spent / x.starting_balance, 2))
...
class EnterpriseOfferFactory(factory.django.DjangoModelFactory):
    ...
    total_discount_ecommerce = factory.lazy_attribute(lambda x: round(x.sum_course_price * (x.discount_value / 100), 2))
    amount_of_offer_spent = factory.lazy_attribute(lambda x: x.total_discount_ecommerce)
    sum_amount_learner_paid = factory.lazy_attribute(lambda x: x.sum_course_price - x.amount_of_offer_spent)
    percent_of_offer_spent = factory.lazy_attribute(lambda x: round(x.amount_of_offer_spent / x.max_discount, 2))
    remaining_balance = factory.lazy_attribute(lambda x: round(x.max_discount - x.amount_of_offer_spent, 2))
    amount_offer_spent_ocm = factory.lazy_attribute(lambda x: round(x.amount_of_offer_spent / 2, 2))
    amount_offer_spent_exec_ed = factory.lazy_attribute(lambda x: round(x.amount_of_offer_spent / 2, 2))
