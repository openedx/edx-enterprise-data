...
def encrypt_string(string):
    ...
    """
    fernet = Fernet(derive_fernet_key(os.environ.get("LMS_FERNET_KEY")))
...
def flatten_dict(d, target="key" or "value"):
    ...
    for key, value in list(OrderedDict(sorted(d.items())).items()):
        ...
        if isinstance(value, dict):
            flattened += list(map(format_nested if target_is_key else lambda x: x, flatten_dict(value, target=target)))
        ...
        # We are suddenly in muddy waters, because lists can have multiple types within them in JSON.
            ...
            if any(items_are_dict) and not all(items_are_dict):
                raise NotImplementedError(
                    "Ability to flatten dict with list of mixed dict and non-dict types is not currently supported"
                )

            # Same here, this is just weird.
            if any(items_are_list):
                raise NotImplementedError(
                    "Ability to flatten a dict with lists within lists "
            ...
            # This case is common, but a little complex.
                ...
                for index, item in enumerate(value):
                    ...
                    if target_is_key:
                        _flattened_dict = [
                            format_nested(flattened_item, _key=index) for flattened_item in _flattened_dict
                        ]
...
def extract_catalog_uuids_from_reporting_config(reporting_config):
    ...
    enterprise_customer_catalogs = {
        "results": [{"uuid": catalog["uuid"]} for catalog in reporting_config.get("enterprise_customer_catalogs", [])]
    }
