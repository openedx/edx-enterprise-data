...
class TestEnterpriseAPIClient(TestCase):
    ...
    def setUp(self):
        ...
        self.api_response = {"results": [{"enterprise_name": "Test Enterprise", "enterprise_id": "test-id"}]}
    ...
    def test_get_all_enterprise_reporting_configs(self, mock_get_oauth_access_token):
        ...
        responses.add(responses.GET, url, json=self.mocked_get_endpoint(), status=200, content_type="application/json")
...
class TestEnterpriseCatalogAPIClient(TestCase):
    ...
    def test_get_customer_catalogs(self, mock_get_oauth_access_token):
        mock_get_oauth_access_token.return_value = ["test_access_token", datetime.now() + timedelta(minutes=60)]
        api_response = {"results": [{"enterprise_name": "Test Enterprise", "enterprise_id": "test-id"}]}
        ...
        responses.add(responses.GET, url, json=mocked_get_endpoint(), status=200, content_type="application/json")
    ...
    def test_get_content_metadata(self, mock_get_oauth_access_token):
        ...
        url = urljoin(self.client.API_BASE_URL, f"{url_path}?page_size=1000")
        responses.add(
            responses.GET, url, json=mocked_get_metadata_endpoint(), status=200, content_type="application/json"
        )
        request_catalogs = {
            "results": [
                {
    ...
    def test_rejected_subject_format_in_content_metadata(self, mock_get_oauth_access_token):
        ...
        try:
            self.client.get_content_metadata(request_catalogs)
