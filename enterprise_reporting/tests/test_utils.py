...
class TestCompressEncrypt(unittest.TestCase):
    ...
    @ddt.data(
        [
            {"name": "lord-of-the-rings.txt", "size": 1000},
        ],
        [
            {"name": "lord-of-the-rings.txt", "size": 1000},
            {"name": "harry-potter-and-deathly-hollows.txt", "size": 500},
        ],
    )
    def test_compress_and_encrypt(self, files_data):
        ...
        compressed_file = utils.compress_and_encrypt([file["file"] for file in files], password)
    ...
    def test_encryption(self, files_data):
        ...
        encrypted_file_name = utils.compress_and_encrypt(
            [file["file"] for file in files], password, str(correct_key.pubkey)
        )
    ...
    @ddt.data("_catalog_json_2019-09-30.json", "_catalog_json.json")
...
class TestPrepareAttachments(unittest.TestCase):
    def test_prepare_attachments(self):
        ...
        for index, filename in enumerate([file_on_filesystem_name, "my_test.csv"]):
            expected_header = f'attachment; filename="{os.path.basename(filename)}"'
